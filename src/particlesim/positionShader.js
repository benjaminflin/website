...
uniform vec2 mousePositionDelta;
...
float snoise2(vec2 p) {
    ...
    return snoise(vec3(p*scale, time*0.005));
} 
...
vec2 gravity(vec2 p) {
    vec2 diff = mousePosition - p;
    float dist = max(length(diff), 0.05);
    return (mousePositionDelta) / (dist * dist);
}
...
void main() {
    ...
    const float gravityScale = 100.0;
    const float boundsBias = 0.1;
